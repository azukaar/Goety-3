...
/**
 * Based on @AlexModGuy's Dreadbow codes: <a href=
 * "https://github.com/AlexModGuy/AlexsCaves/blob/main/src/main/java/com/github/alexmodguy/alexscaves/server/item/DreadbowItem.java">...</a>
 */
public class ArrowRainSpell extends EverChargeSpell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        int maxHeight = 15;
        int i = 0;
        while (mutableBlockPos.getY() < worldIn.getMaxBuildHeight() && worldIn.isEmptyBlock(mutableBlockPos)
                && i < maxHeight) {
        ...
        for (int j = 0; j < potency + 1; ++j) {
            ...
            int clearTries = 0;
            while (clearTries < 6 && !worldIn.isEmptyBlock(BlockPos.containing(vec3))
                    && worldIn.getFluidState(BlockPos.containing(vec3)).isEmpty()) {
                clearTries++;
            ...
            if (!worldIn.isEmptyBlock(BlockPos.containing(vec3))
                    && worldIn.getFluidState(BlockPos.containing(vec3)).isEmpty()) {
                vec3 = mutableBlockPos.getCenter();
            }
            worldIn.sendParticles(new FoggyCloudParticleOption(new ColorUtil(0x4b4b4b), 1.5F, 6), vec3.x(), vec3.y(),
                    vec3.z(), 1, 0, 0, 0, 0);
            arrow.setPos(vec3);
            ...
            if (worldIn.addFreshEntity(arrow)) {
                this.playSound(worldIn, arrow, SoundEvents.ARROW_SHOOT, 2.0F,
