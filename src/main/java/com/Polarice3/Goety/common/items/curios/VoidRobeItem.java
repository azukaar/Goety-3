        ...
        if (!worldIn.isClientSide) {
            if (entityIn instanceof LivingEntity livingEntity) {
                if (CuriosFinder.hasVoidRobe(livingEntity)){
                    ...
                    if (com.Polarice3.Goety.utils.ConfigHelper.getInt(ItemConfig.VoidRobeWaterSapped, 0) > 0) {
                        if (livingEntity.isInWaterRainOrBubble()) {
                            livingEntity.addEffect(new MobEffectInstance(GoetyEffects.SAPPED, 10, com.Polarice3.Goety.utils.ConfigHelper.getInt(ItemConfig.VoidRobeWaterSapped, 0) - 1, false, false));
                        }
                    }
                }
            }
