...
public class BrewBagItemCapability implements ICapabilitySerializable<Tag> {
    private final ItemStack stack;
    private final LazyOptional<IItemHandler> holder = LazyOptional.of(this::getHandler);
    private BrewBagItemHandler handler;

    public BrewBagItemCapability(ItemStack stack) {
        this.stack = stack;
    }

    @Nonnull
    private BrewBagItemHandler getHandler() {
        if (handler == null) {
            handler = new BrewBagItemHandler(stack);
        }
        return handler;
    }
